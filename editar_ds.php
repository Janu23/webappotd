<?php 
    ...
    //input diagnostico trocando virgula por ponto
    if (isset($_POST['extensaoReparo'])){
        $extensaoReparo = $_POST['extensaoReparo'];
        $extensaoReparo = floatval(str_replace(',', '.', $extensaoReparo));
    }
    if (isset($_POST['extensaoLimpeza'])){
        $extensaoLimpeza = $_POST['extensaoLimpeza'];
        $extensaoLimpeza = floatval(str_replace(',', '.', $extensaoLimpeza));
    }
    if (isset($_POST['extensaoImplantar'])){
        $extensaoImplantar = $_POST['extensaoImplantar'];
        $extensaoImplantar = floatval(str_replace(',', '.', $extensaoImplantar));
    }
      
    //checkbox diagnóstico
    $reparar = (isset($_POST['reparar'])) ? "Sim" : "Não";
    $limpeza = (isset($_POST['limpeza'])) ? "Sim" : "Não";
    $implantar = (isset($_POST['implantar'])) ? "Sim" : "Não";
    $ok = (isset($_POST['ok'])) ? "Sim" : "Não";
    ...
    //O status da edição leva em consideração as 3 var abaixo
    //0-> Não editado/ 1-> Envio sem fotos/ 2-> Apenas uma foto enviada/ 3-> Edição completa
    $edit = 1;

    $destination = $_POST['foto1dir'];
    $destination2 = $_POST['foto2dir'];
   ...
   //Foto1
    ...
    if (isset($_FILES ['foto2_ficha']) && !empty($_FILES["foto2_ficha"]["name"])){
        ...
        $newName2 = $identificacao;
        $destination2 = "{$diretorio}/{$newName2}_foto2.{$extension2}";
